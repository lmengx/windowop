<template>
    ...
    <div v-if="editID === 0">
        <h2>检测规则</h2>
        <button class="btn" @click="queryTargetList(currentPage)">刷新</button>
        <button class="btn" @click="startCreating">添加规则</button>
        ...
        <ul>
            <li v-for="target in paginatedTargets" :key="target.ID">
                <div>
                    ...
                    <button class="btn" @click="startEdit(target)">编辑</button>
                    <button class="btn" v-show="target.Enable == '0'" @click="toggleEnable(target, '1')">启用</button>
                    <button class="btn" v-show="target.Enable == '1'" @click="toggleEnable(target, '0')">禁用</button>
                    <button class="btn" @click="deleteTarget(target.ID)">X</button>
                </div>
            </li>
        </ul>

        <div class="pagination" v-if="totalPages > 1 && editID === 0">
