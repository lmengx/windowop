    ...
    static void SendMsg(string msg, WebSocket ws, string ip)
        ...
        async private static void HandleWebSocket(object state)
        {
            try
            {
                ...
                if (context.Request.Url.AbsolutePath.Equals("/websockify", StringComparison.OrdinalIgnoreCase))
                {

                    // 3. 连接到目标 VNC 服务器的 TCP 套接字
                    using (var vncSocket = new System.Net.Sockets.TcpClient("127.0.0.1", 5900))
                    using (var vncStream = vncSocket.GetStream())
                    {
                        // 4. 创建双向转发任务
                        var webSocketToVnc = ForwardWebSocketToTcp(webSocket, vncStream);
                        var vncToWebSocket = ForwardTcpToWebSocket(vncStream, webSocket);

                        // 5. 等待任一任务完成（任意一方关闭连接时终止）
                        await Task.WhenAny(webSocketToVnc, vncToWebSocket);
                    }
                }
        ...
        // 将 WebSocket 数据转发到 TCP (VNC)
        private static async Task ForwardWebSocketToTcp(WebSocket webSocket, System.IO.Stream tcpStream)
        {
            ...
            while (webSocket.State == WebSocketState.Open)
            {
                ...
                await tcpStream.WriteAsync(buffer, 0, receiveResult.Count);
            }
        }

        // 将 TCP (VNC) 数据转发到 WebSocket
        private static async Task ForwardTcpToWebSocket(System.IO.Stream tcpStream, WebSocket webSocket)
        {
            byte[] buffer = new byte[8192];
            while (webSocket.State == WebSocketState.Open)
            {
                int bytesRead = await tcpStream.ReadAsync(buffer, 0, buffer.Length);
                if (bytesRead == 0) break; // TCP 连接关闭
